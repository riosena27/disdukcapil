    ...
    <div class="col s12">
        <div class="card">
            <div class="card-content">
                <div class="row">
                    <a href="{{url('kadis')}}" class="waves-effect waves-light btn red left"><i
                ...
                <span class="card-title">Review Akta Kelahiran <button
                    ...
                    <form action="{{url('kadis/akta-kelahiran/review-checked')}}" method="POST" id="giveStatus">
                        @method('put')
                        @csrf
                        <span id="statusKadis"></span>
                    ...
                    <tbody>
                        @foreach ($aktaUser as $item)
                            <tr>
                                ...
                                <td align="center">
                                    <a href="{{url('kadis/akta-kelahiran/'.$item->id)}}" class="waves-effect waves-light btn blue tooltipped btn-small" data-position="top" data-tooltip="Review Akta Kelahiran">Review Data<i class="material-icons right">folder_open</i></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
...
<script>
    $(document).ready(function () {
            $('.setujui').click(function () {
                ...
                $("#statusKadis").html('<input type="hidden" name="statusKadis" value="3">');
            });
            ...
            $('.tolak').click(function () {
                ...
                $("#modal").find("h4").text(header);
                $("#modal").find("#statusContent").text(content);
                $("#statusKadis").html('<input type="hidden" name="statusKadis" value="2">');
            });
        });
</script>
@endsection